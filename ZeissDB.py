...
# -rd option: folder for the cmm result file
...
if __name__ == "__main__":
    ...
    try:
        ...
        dialogFile = pd+planid+"\\dialog.xml"
        if os.path.isfile(dialogFile):
            dialogData = zrl.loadDialog(dialogFile)
        else:
            print("dialog file not found: " + dialogFile)
            input()
            exit()
        ...
        if os.path.isfile(resultFile):
            ...
            temp.append({"resultID":len(temp),"date": now.strftime("%Y%m%d"), "time": now.strftime("%H:%M:%S"),"headerData": dialogData,"chrData": chrData})
            with open(resultFile, 'w') as file:
                json.dump(temp, file, indent=2)
            
            # jstr = json.dumps(temp, indent=2)
            # zstr = bz2.compress(jstr.encode(), 5)

            # with open(resultFile+".zcmm", 'wb') as file2:
            #     file2.write(zstr)
            ...
            # jstr = json.dumps(results, indent=2)
            # zstr = bz2.compress(jstr.encode(), 9)
